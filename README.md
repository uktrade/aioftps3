...
- an ACME account key [which is an SSL key],
- SSL key(s) for the certificate(s),
- CSR(s) for the domain(s).

You can see examples of how to generate these in [minio-entrypoint.sh](./minio-entrypoint.sh), which is used when running automated tests.
...
| `AWS_S3_BUCKET__REGION` | The region of the S3 bucket that stores the files. | `eu-west-1` |
| `AWS_S3_BUCKET__HOST` | The hostname used to communicate with S3. | `s3-eu-west-1.amazonaws.com` |
| `AWS_S3_BUCKET__NAME` | The name of the bucket files are stored in. | `my-bucket-name` |
| `AWS_S3_BUCKET__DIR_SUFFIX` | The suffix of the keys created in order to simulate a directory. Must start with a forward slash, but does not need to be longer.  | `/` |
| `AWS_S3_BUCKET__VERIFY_CERTS` | Whether to verify the SSL certificates when communicating with S3. | `true` |
| `AWS_S3_ACME_BUCKET__REGION` | The region of the S3 bucket that stores the ACME-related fiels: account key, SSL keys, certificates, and CSRs. | `eu-west-1` |
| `AWS_S3_ACME_BUCKET__HOST` | The hostname used to communicate with S3 for ACME-related files.  | `s3-eu-west-1.amazonaws.com` |
| `AWS_S3_ACME_BUCKET__NAME` | The name of the S3 bucket that stores ACME-related files. | `my-bucket-name` |
| `AWS_S3_ACME_BUCKET__VERIFY_CERTS` |  Whether to verify the SSL certificates when communicating with S3. | `/`
| `AWS_ROUTE_53__HOST` | The hostname used to communicate with Route 53 | `route53.amazonaws.com` |
| `AWS_ROUTE_53__REGION` | The region where Route 53 resides. | `us-east-1` |
| `AWS_ROUTE_53__VERIFY_CERTS` | Whether to verify SSL certificates when connecting to Route 53. | `true` |
| `AWS_ROUTE_53__ZONE_ID` | The Zone ID where ACME challenge TXT records are created. | `ommitted` |
| `ACME_DIRECTORY` | The URL of the ACME directory service | `https://acme-v02.api.letsencrypt.org/directory` |
| `ACME_LOCAL_PATH` | Where to store ACME-related files locally | `/home/ftps` |
...
docker run --rm -p 8021-8042:8021-8042 \
  ...
  -e AWS_S3_BUCKET__VERIFY_CERTS=true \
  ...
  -e AWS_S3_ACME_BUCKET__VERIFY_CERTS=true \
  -e AWS_ROUTE_53__HOST=route53.amazonaws.com \
  -e AWS_ROUTE_53__REGION=us-east-1 \
  -e AWS_ROUTE_53__VERIFY_CERTS=true \
  -e AWS_ROUTE_53__ZONE_ID=ommitted \
  -e ACME_DIRECTORY=https://acme-staging-v02.api.letsencrypt.org/directory \
  -e ACME_LOCAL_PATH=/home/ftps \
